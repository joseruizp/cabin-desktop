...
public class PWDialog extends JDialog implements ActionListener {
	...
	public PWDialog() {
		...
		pwMainPanel.add(Box.createRigidArea(
				new Dimension(getToolkit().getScreenSize().width, getToolkit().getScreenSize().height / 2 - 300)));
	...
	private void startWebSocket() {
		server = new Server("localhost", 8090, "/websockets", new HashMap<String, Object>(),
				RechargeMessageEndpoint.class);
		try {
			server.start();
